...
高阶组件可以看作 React 对装饰模式的一种实现，高阶组件就是一个函数，且该函数接受一个组件作为参数，并返回一个新的组件，他会返回一个增强的 React 组件，高阶组件可以让我们的代码更具有复用性，逻辑性与抽象性，可以对 render 方法进行劫持，也可以控制 props 与 state 等。高阶组件就是一个没有副作用的纯函数。
