...
// handerClick 的打印输出：1 0 -> 2 0 -> 3 0 -> 4 3 -> 5 3
// 最终的打印：render 0 -> 1 0 -> 2 0 -> 3 0 -> render 3 -> 4 3 -> 5 3 -> render 1
```
...
```jsx
// api
[ state , dispatchAction ] = useState(initData)
...
console.log(arr); // 打印（ [] !== []，内存地址变了，触发render）
return <div onClick={test}>App</div>
```
...
- 原因：在 `useState` 的 `dispatchAction` 处理逻辑中，会**浅比较前后两次 state** ，发现 state 相同，不会开启更新调度任务。
...
// 结论：与 setState 一样，三次 setState 只触发了一次 render，此条件下 useState 是异步的，批量的，合并 state，然后触发 render 函数，得到新的 UI 视图层，完成 render 阶段。
...
// 结论：异步操作里面的批量更新规则会被打破。与 setState 一样，在 setTimeout 里，useState 是同步的，非批量的，每次 setState 后 state 马上变，每次修改 state 都会 render。
```
